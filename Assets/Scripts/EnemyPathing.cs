...
public class EnemyPathing : MonoBehaviour {
	...
	void Start () {
        localHealth = GameMaster.healthPoints * DifficultyControl.healthMultiplier;   
	}
    ...
    void OnTriggerEnter(Collider other)
    {
        if (other.name == "Projectile(Clone)" || other.name == "ProjectileRed(Clone)")
        {
            ...
            {              
                ...
                GameMaster.playerScore += 10 * GameMaster.healthPoints * DifficultyControl.healthMultiplier;
                LevelSystem.playerXP += 10 * GameMaster.healthPoints * DifficultyControl.healthMultiplier;
                PlayerPrefs.SetFloat("Player XP", LevelSystem.playerXP);
            }
            else
            {
